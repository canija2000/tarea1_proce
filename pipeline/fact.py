...
def build_fact_news(
    ...
    try:
        ...
        with tqdm(desc="  fact_news", unit=" rows", unit_scale=True, dynamic_ncols=True) as pbar:
            for chunk_num, chunk in enumerate(reader, start=1):
                ...
                # actualizamos la cantidad de filas escritas y la barra de progreso
                chunk_len = len(chunk)
                rows_written += chunk_len
                pbar.update(chunk_len)
                elapsed = time.time() - t_start
                rate    = rows_written / elapsed if elapsed > 0 else 0
                pbar.set_postfix({"descartadas": rows_skipped, "vel": f"{rate:,.0f} f/s"})
